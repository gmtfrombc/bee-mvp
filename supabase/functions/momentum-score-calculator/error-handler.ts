...
// Minimal structural typing for the Supabase client used here
type QueryBuilder = {
  select: (
    ...args: unknown[]
  ) => QueryBuilder & Promise<{ data: unknown[] | null; error: Error | null }>
  insert: (
    ...args: unknown[]
  ) => QueryBuilder & Promise<{ data: unknown[] | null; error: Error | null }>
  upsert: (
...
export class MomentumErrorHandler {
  private supabase: DBSupabaseClient

  constructor(supabaseClient: DBSupabaseClient) {
   ...
   */
  validateEngagementEvents(events: unknown[]): ValidationResult {
    const errors: string[] = []
    const warnings: string[] = []
    ...
    events.forEach((event, index) => {
      ...
      if (typeof event !== 'object' || event === null) {
        errors.push(`Event ${index}: must be an object`)
        return
      }

      const typedEvent = event as Record<string, unknown>

      if (!typedEvent.event_type) {
      ...
      if (
        typedEvent.points !== undefined &&
        (typeof typedEvent.points !== 'number' || typedEvent.points < 0)
      ) {
  ...
  async checkRateLimit(
    ...
    try {
      ...
      const { count, error } = await (this.supabase
        ...
        .gte('created_at', windowStart.toISOString())) as unknown as {
          count: number | null
          error: Error | null
        }
  ...
  async getSystemHealth(): Promise<{
    ...
    try {
      ...
      const { data: errorStats } = await this.supabase
        .rpc('get_error_statistics', { p_hours_back: 1 }) as unknown as {
          data: { total_errors?: number; critical_errors?: number } | null
        }
