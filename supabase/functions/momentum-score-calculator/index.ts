...
class MomentumScoreCalculator {
    ...
    constructor(supabaseUrl: string, supabaseKey: string) {
        ...
        this.errorHandler = new MomentumErrorHandler(supabaseUrl, supabaseKey)
    }
    ...
    async calculateMomentumScore(userId: string, targetDate: string): Promise<Response> {
        return await this.errorHandler.withErrorHandling(async () => {
            ...
            // Sanitize inputs
            const sanitizedUserId = this.errorHandler.sanitizeInput(userId)
            const sanitizedDate = this.errorHandler.sanitizeInput(targetDate)
            ...
            try {
                ...
                const { data: savedScore, error: saveError } = await this.supabase
    ...
    async handleBatchCalculation(request: Request): Promise<Response> {
        try {
            ...
            const { user_ids, target_date, force_recalculate = false, batch_size = 10 } = body
    ...
    async calculateUserMomentumScore(userId: string, targetDate: string): Promise<DailyEngagementScore> {
        ...
        const momentumState = await this.classifyMomentumState(userId, normalizedScore, historicalScores)
     ...
     * Get historical scores for trend analysis
     */
    private async getHistoricalScores(userId: string, beforeDate: string, days: number): Promise<DailyEngagementScore[]> {
     ...
     */
    private applyExponentialDecay(rawScore: number, historicalScores: DailyEngagementScore[], targetDate: string): number {
     ...
     */
    private async classifyMomentumState(
        userId: string,
        score: number,
        historicalScores: DailyEngagementScore[]
    ): Promise<'Rising' | 'Steady' | 'NeedsCare'> {
        ...
        if (currentState) {
            ...
            if (currentState === 'Rising' && score >= MOMENTUM_CONFIG.RISING_THRESHOLD - buffer) {
                newState = 'Rising'
            } else if (currentState === 'NeedsCare' && score <= MOMENTUM_CONFIG.NEEDS_CARE_THRESHOLD + buffer) {
                newState = 'NeedsCare'
            }
        }
     ...
     */
    private createScoreBreakdown(events: EngagementEvent[], rawScore: number, finalScore: number): Record<string, any> {
...
serve(async (request: Request) => {
    ...
    const calculator = new MomentumScoreCalculator(supabaseUrl, supabaseKey)
