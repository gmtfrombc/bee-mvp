...
const mockSystemTemplate = 'Coach persona: {{persona}}\nMomentum: {{momentum_state}}\nEngagement: {{engagement_summary}}'

// Mock Deno.readTextFile
const originalReadTextFile = Deno.readTextFile
globalThis.Deno.readTextFile = async (path: string | URL) => {
    const pathStr = typeof path === 'string' ? path : path.toString()
    if (pathStr.includes('safety.md')) {
        return mockSafetyTemplate
    }
    if (pathStr.includes('system.md')) {
        return mockSystemTemplate
    }
...
Deno.test('Prompt Builder - Positive sentiment integration', async () => {
    ...
    const prompt = await buildPrompt(
        ...
        'challenging',
        { engagementPeaks: ['morning'], volatilityScore: 0.3 },
        'Rising',
        [],
        undefined,
        sentimentResult
...
Deno.test('Prompt Builder - Negative sentiment integration', async () => {
    ...
    const prompt = await buildPrompt(
        ...
        'supportive',
        { engagementPeaks: ['evening'], volatilityScore: 0.5 },
        'NeedsCare',
        [],
        undefined,
        sentimentResult
...
Deno.test('Prompt Builder - Neutral sentiment integration', async () => {
    ...
    const prompt = await buildPrompt(
        ...
        'educational',
        { engagementPeaks: ['afternoon'], volatilityScore: 0.2 },
        'Steady',
        [],
        undefined,
        sentimentResult
...
Deno.test('Prompt Builder - Borderline negative sentiment', async () => {
    ...
    const prompt = await buildPrompt(
        'Not feeling great',
        'supportive',
        { engagementPeaks: ['evening'], volatilityScore: 0.4 },
        'NeedsCare',
        [],
        undefined,
        sentimentResult
