...
export class EffectivenessTracker {
  ...
  constructor(_supabaseUrl?: string, overrideKey?: string) {
    // Initialize client lazily without adding static dependency
    ;(async () => {
  ...
  async analyzeUserEffectiveness(
    ...
    try {
      ...
      const helpfulRate = totalInteractions > 0 ? helpfulInteractions.length / totalInteractions : 0
      ...
      const recommendedPersona = Object.entries(personaEffectiveness)
        .reduce(
          (best, [persona, score]) => score > personaEffectiveness[best] ? persona : best,
          'supportive',
        )
  ...
  async getEffectivenessSummary(userId: string): Promise<{
    ...
    try {
      ...
      const effectivenessDiff = currentAnalysis.overallEffectiveness -
        previousAnalysis.overallEffectiveness
      ...
      return {
        ...
        helpfulPercentage: Math.round(
          (Object.values(currentAnalysis.personaEffectiveness).reduce((sum, v) => sum + v, 0) / 3) *
            100,
        ),
