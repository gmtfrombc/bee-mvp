...
export class StrategyOptimizer {
    ...
    private generateAdaptationReasons(
        ...
        if (effectiveness.personaEffectiveness[selectedPersona] > 0.7) {
            reasons.push(`Using ${selectedPersona} persona - high effectiveness score (${Math.round(effectiveness.personaEffectiveness[selectedPersona] * 100)}%)`);
        ...
        if (frequency === 'low' && effectiveness.responseRate < 0.3) {
            reasons.push(`Reduced frequency due to low response rate (${Math.round(effectiveness.responseRate * 100)}%)`);
    ...
    async shouldUpdateStrategy(
        ...
        try {
            ...
            const recentEffectiveness = await this.effectivenessTracker.analyzeUserEffectiveness(userId, 3);
            ...
            const currentPersonaScore = recentEffectiveness.personaEffectiveness[currentStrategy.preferredPersona];
            const recommendedPersona = recentEffectiveness.recommendedPersona;

            if (recommendedPersona !== currentStrategy.preferredPersona &&
            ...
            return { shouldUpdate: false, reasons: ['Error checking strategy - maintaining current approach'] };
        }
    }
} 