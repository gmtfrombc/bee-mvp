...
export function analyzeEngagement(events: EngagementEvent[]): PatternSummary {
  ...
  // Calculate engagement frequency based on event density
  const engagementFrequency = calculateEngagementFrequency(sortedEvents)
...
function createRollingWindows(
  events: Array<EngagementEvent & { parsedTimestamp: Date }>,
  windowDays: number,
): TimeWindow[] {
  ...
  for (
    let startTime = earliestDate.getTime();
    startTime <= latestDate.getTime();
    startTime += millisecondsPerDay
  ) {
...
function analyzeEngagementPeaks(
  events: Array<EngagementEvent & { parsedTimestamp: Date }>,
): string[] {
  ...
  const variance =
    hourlyEngagement.reduce((acc, count) => acc + Math.pow(count - avgEventsPerHour, 2), 0) / 24
...
function calculateVolatilityScore(windows: TimeWindow[]): number {
  ...
  const variance = engagementRates.reduce((acc, rate) => acc + Math.pow(rate - mean, 2), 0) /
...
function calculateEngagementFrequency(
  events: Array<EngagementEvent & { parsedTimestamp: Date }>,
): 'low' | 'medium' | 'high' {
  if (events.length === 0) return 'low'

  // Calculate events per day over the entire period
  const earliestDate = events[0].parsedTimestamp
  const latestDate = events[events.length - 1].parsedTimestamp
  const daySpan = Math.max(
    1,
  ...
  // Categorize based on events per day
  // TODO: Calibrate thresholds based on real user engagement data
