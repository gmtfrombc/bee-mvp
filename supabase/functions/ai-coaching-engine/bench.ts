...
class CoachBenchmark {
  ...
  private async workerLoop(
    ...
    while (Date.now() < endTime) {
      ...
      try {
        ...
        this.errors.push(`Network error from worker ${workerId}: ${msg}`)
      }
    }
  ...
  printResults(results: BenchmarkResult): void {
    ...
    console.log(
      `Success Rate: ${((results.successCount / results.totalRequests) * 100).toFixed(2)}%`,
    )
    ...
    console.log(
      `P95: ${results.p95Latency.toFixed(2)}ms ${
        results.p95Latency > 1200 ? '❌ EXCEEDS TARGET' : '✅'
      }`,
    )
    ...
    console.log(
      `Memory Budget: ${totalMemoryMB > 40 ? '❌ EXCEEDS 40MB LIMIT' : '✅ WITHIN BUDGET'}`,
    )
  ...
  async runSingleTest(targetUrl?: string): Promise<void> {
    ...
    try {
      ...
      console.log(`❌ Network error: ${_err instanceof Error ? _err.message : String(_err)}`)
    }
  }
}
