...
class TestPerformanceOptimization:
    ...
    def test_user_date_index_performance(self, db_connection, test_data_setup):
        ...
        assert (
            query_time < TEST_CONFIG["performance_thresholds"]["single_user_query_ms"]
        )
        ...
        assert (
            "idx_daily_scores_user_date" in explain_text or "Index Scan" in explain_text
        )
    ...
    def test_recent_scores_partial_index_performance(
        self, db_connection, test_data_setup
    ):
    ...
    def test_materialized_view_refresh_performance(
        self, db_connection, test_data_setup
    ):
        ...
        assert (
            refresh_time_ms
            < TEST_CONFIG["performance_thresholds"]["materialized_view_refresh_ms"]
        )
    ...
    def test_recent_user_momentum_view_performance(
        self, db_connection, test_data_setup
    ):
    ...
    def test_intervention_candidates_view_performance(
        self, db_connection, test_data_setup
    ):
    ...
    def test_analyze_momentum_tables_performance(self, db_connection, test_data_setup):
        ...
        assert (
            analysis_time_ms
            < TEST_CONFIG["performance_thresholds"]["maintenance_operation_ms"]
        )
    ...
    def test_concurrent_user_queries_performance(self, db_connection, test_data_setup):
        ...
        test_users = test_data_setup["test_users"][
            :20
        ]  # Use 20 users for concurrent testing
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(query_user_momentum, user_id) for user_id in test_users
            ]
        ...
        assert (
            avg_query_time
            < TEST_CONFIG["performance_thresholds"]["single_user_query_ms"] * 2
        )
        # Allow 3x for worst case
        assert (
            max_query_time
            < TEST_CONFIG["performance_thresholds"]["single_user_query_ms"] * 3
        )

    def test_concurrent_analytics_queries_performance(
        self, db_connection, test_data_setup
    ):
        ...
        assert (
            avg_query_time
            < TEST_CONFIG["performance_thresholds"]["complex_analytics_ms"]
        )
        assert (
            max_query_time
            < TEST_CONFIG["performance_thresholds"]["complex_analytics_ms"] * 2
        )
    ...
    def test_performance_baseline_establishment(self, db_connection, test_data_setup):
        ...
        for query_test in baseline_queries:
            query_time = self.measure_query_time(
                cursor, query_test["query"], query_test["params"]
            )
            ...
            assert (
                query_time < query_test["threshold_ms"]
