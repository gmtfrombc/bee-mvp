...
class PerformanceTester:
    ...
    def _log_test_result(self, test_name: str, passed: bool, details: str = "", metrics: Dict = None):
            ...
            for i in range(num_tests):
                ...
                with conn.cursor() as cur:
                    cur.execute("""
                        INSERT INTO engagement_events (user_id, event_type, value) 
                        VALUES (%s, %s, %s)
                    """, (self.test_user_id, f'perf_test_single_{i}',
    ...
    def test_concurrent_insert_performance(self) -> bool:
        ...
        try:
                    ...
                    for i in range(inserts_per_thread):
                        ...
                        with conn.cursor() as cur:
                            cur.execute("""
                                INSERT INTO engagement_events (user_id, event_type, value) 
                                VALUES (%s, %s, %s)
                            """, (self.test_user_id, f'perf_test_concurrent_{thread_id}_{i}',
                                  json.dumps({
            ...
            with ThreadPoolExecutor(max_workers=num_threads) as executor:
                futures = [executor.submit(insert_events, i)
            ...
            if all_times:
                ...
                throughput = len(all_times) / \
                    (total_duration / 1000)  # inserts per second
                ...
                details = f"Concurrent inserts: {len(all_times)}, " \
                    f"Avg time: {avg_time:.2f}ms, Throughput: {throughput:.2f}/sec"

                self._log_test_result(
                    "Concurrent Insert Performance", performance_passed, details, metrics)
                return performance_passed
            else:
                self._log_test_result(
    ...
    def test_query_performance_with_indexes(self) -> bool:
        ...
        try:
            ...
            jsonb_uses_index = 'Bitmap' in str(
                jsonb_plan) or 'Index Scan' in str(jsonb_plan)

            # Performance criteria: queries should be fast and use indexes
            performance_passed = (
                timeline_time < 50.0 and  # Timeline query < 50ms
                event_type_time < 100.0 and  # Event type query < 100ms
                jsonb_time < 200.0 and  # JSONB query < 200ms
                timeline_uses_index and
                event_type_uses_index
            )
            ...
            details = f"Timeline: {timeline_time:.2f}ms, Event type: {event_type_time:.2f}ms, " \
    ...
    def test_large_dataset_performance(self) -> bool:
        ...
        try:
            ...
            queries = [
                ("Recent events", "SELECT * FROM engagement_events WHERE user_id = %s ORDER BY timestamp DESC LIMIT 100"),
                ("Date range", "SELECT COUNT(*) FROM engagement_events WHERE user_id = %s AND timestamp >= %s"),
                ("Event type filter", "SELECT COUNT(*) FROM engagement_events WHERE user_id = %s AND event_type LIKE 'app_%'"),
                ("JSONB aggregation",
        ...
        try:
            ...
            for i in range(num_tests):
                ...
                with conn.cursor() as cur:
                    ...
                    """, (self.test_user_id, 'realtime_test',
