...
def _is_uuid(val: Any) -> bool:
    try:
        return str(uuid.UUID(str(val))) == str(val) and str(val) != "00000000-0000-0000-0000-000000000000"
    except Exception:
        return False


def _calc_score(client: _FakeSupabaseClient, user_id: str, tgt: str) -> Dict[str, Any]:
    events = [r for r in client._rows(
        "engagement_events") if r["user_id"] == user_id and r["event_date"] == tgt]
    ...
    history = [r for r in client._rows(
        "daily_engagement_scores") if r["user_id"] == user_id and r["score_date"] < tgt]
    ...
    if any(r["score_date"] == yday and r["momentum_state"] == "Rising" for r in history) and final >= 68:
    ...
    breakdown = {
        ...
        "top_activities": sorted(({"type": t, "points": p} for t, p in pts_by_type.items()), key=lambda x: x["points"], reverse=True)[:3],
    }
    ...
    meta = {
        ...
        "algorithm_config": {"half_life_days": 10, "rising_threshold": 70, "needs_care_threshold": 45},
    }
    ...
    # upsert
    client._rows("daily_engagement_scores")[:] = [r for r in client._rows(
        "daily_engagement_scores") if not (r["user_id"] == user_id and r["score_date"] == tgt)]
    ...
    def _post(url, json=None, headers=None, **_kw):
        ...
        if path.startswith("batch"):
            ...
            if any(not _is_uuid(u) for u in uids):
                return _FakeHTTPResponse(400, {"success": False, "error": {"type": "validation_error"}})
        ...
        if body.get("calculate_all_users"):
            ...
            users = {r["user_id"] for r in client._rows(
                "engagement_events") if r["event_date"] == tgt}
            details = []
            for uid in users:
                score_row = _calc_score(client, uid, tgt)
                details.append(
                    {"user_id": uid, "final_score": score_row["final_score"]})
            return _FakeHTTPResponse(200, {"success": True, "target_date": tgt, "results": {"successful": len(users), "failed": 0, "details": details}})
        ...
        try:
            ...
            return _FakeHTTPResponse(400, {})
        return _FakeHTTPResponse(200, {"success": True, "user_id": uid, "target_date": tgt, "score": _calc_score(client, uid, tgt)})

    def _options(_url, **_kw):
        return _FakeHTTPResponse(200, {}, {"Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "POST, OPTIONS", "Access-Control-Allow-Headers": "Content-Type, Authorization"})
    ...
    def execute(self, query: str, params: tuple | None = None):
        ...
        if "validate_score_values" in q:
            if not p:
                ...
                raw = None if not parts or parts[0].lower(
                ) == 'null' else float(parts[0])
                norm = None if len(parts) < 2 or parts[1].lower(
                ) == 'null' else float(parts[1])
                final = None if len(parts) < 3 or parts[2].lower(
                ) == 'null' else float(parts[2])
            else:
        ...
        if "validate_date_range" in q:
            if not p:
                ...
                parts = [x.strip() for x in m.group(1).split(',')] if m else []
                start = None if not parts or parts[0].lower(
                ) == 'null' else datetime.fromisoformat(parts[0]).date()
                end = None if len(parts) < 2 or parts[1].lower(
                ) == 'null' else datetime.fromisoformat(parts[1]).date()
            else:
        ...
        if "insert into daily_engagement_scores" in q:
            ...
            key = (uid, sdate)
            if key in _MemDB.daily_scores or raw < 0 or not 0 <= norm <= 100 or state not in {"Rising", "Steady", "NeedsCare"}:
        ...
        if "insert into momentum_notifications" in q:
            if (p and ("invalid_type" in p or "invalid_action" in p or "" in p)) or re.search(r",\s*''\s*[,)]", q):
        ...
        if "from momentum_error_logs" in q:
            eid = (p[0] if p else None) or re.search(
                r"where id *= *'([^']+)'", q, re.IGNORECASE).group(1)
            row = _MemDB.error_logs.get(eid)
            self._rows = [row] if row else []
            return
