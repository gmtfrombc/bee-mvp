...
class TestDataValidationErrorHandling:
    ...
    def test_daily_engagement_scores_validation_trigger(
        self, db_connection, test_user_id
    ):
    ...
    def test_momentum_notifications_validation_trigger(
        self, db_connection, test_user_id
    ):
        ...
        with pytest.raises(psycopg2.Error):
            ...
            """,
                (
                    test_user_id,
                    "invalid_type",
                    "2024-12-17",  # Invalid type
                    "",
                    "Message",  # Empty title
                    "invalid_action",
                    "pending",  # Invalid action
                ),
            )
        ...
        with pytest.raises(psycopg2.Error):
            ...
            """,
                (
                    ...
                    "",
                    "invalid_status",
                    "2024-12-18",  # Empty reason, invalid status
                ),
            )
    ...
    def test_safe_calculate_momentum_score_valid_input(
        self, db_connection, test_user_id
    ):
    ...
    def test_check_momentum_system_health(self, db_connection):
        ...
        assert health["health"]["status"] in ["healthy", "degraded", "critical"]
    ...
    def test_edge_function_health_check(self):
        ...
        try:
            response = requests.get(
                f"{TEST_CONFIG['edge_function_url']}/health", timeout=10
            )
