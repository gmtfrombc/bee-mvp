...
class APIValidationTester:
    ...
    def _log_test_result(self, test_name: str, passed: bool, details: str = "", response_data: Dict = None):
    ...
    def _make_rest_request(self, method: str, endpoint: str, auth_type: str = 'user',
    ...
    def _make_graphql_request(self, query: str, variables: Dict = None, auth_type: str = 'user') -> Tuple[int, Dict]:
    ...
    def test_rest_api_crud_operations(self) -> bool:
        ...
        try:
            ...
            created_event_id = response[0]['id'] if create_success and response else None
            ...
            if created_event_id:
                update_data = {'value': {'updated': True,
                                         'test_id': test_event['value']['test_id']}}
                status, response = self._make_rest_request(
                    'PATCH', f'engagement_events?id=eq.{created_event_id}', 'user', update_data)
            ...
            crud_passed = create_success and read_all_success and read_specific_success and update_handled and delete_handled

            details = f"CREATE: {create_success}, READ_ALL: {read_all_success}, " \
                f"READ_SPECIFIC: {read_specific_success}, UPDATE: {update_handled}, DELETE: {delete_handled}"

            self._log_test_result(
                "REST API CRUD Operations", crud_passed, details)
            return crud_passed

        except Exception as e:
            self._log_test_result(
                "REST API CRUD Operations", False, f"Error: {str(e)}")
            return False
    ...
    def test_graphql_api_operations(self) -> bool:
        ...
        try:
            ...
            status, response = self._make_graphql_request(
                query, auth_type='user')
            query_success = status == 200 and 'data' in response and 'engagement_events' in response[
                'data']
            ...
            mutation_success = status == 200 and 'data' in response and 'insert_engagement_events_one' in response[
                'data']
            ...
            self._log_test_result("GraphQL API Operations",
                                  graphql_passed, details)
    ...
    def test_unauthorized_access_responses(self) -> bool:
        ...
        try:
            ...
            status, response = self._make_graphql_request(
                query, auth_type='none')
            graphql_auth_denied = status in [401, 403]

            auth_passed = no_auth_denied and invalid_jwt_denied and cross_user_denied and graphql_auth_denied

            details = f"No auth: {no_auth_denied}, Invalid JWT: {invalid_jwt_denied}, " \
                f"Cross-user: {cross_user_denied}, GraphQL: {graphql_auth_denied}"

            self._log_test_result(
                "Unauthorized Access Responses", auth_passed, details)
            return auth_passed
    ...
    def test_service_role_access(self) -> bool:
        ...
        try:
            ...
            service_passed = service_read_success and service_insert_success and bulk_insert_success

            details = f"Read: {service_read_success}, Insert: {service_insert_success}, " \
                f"Bulk: {bulk_insert_success}"

            self._log_test_result("Service Role Access",
                                  service_passed, details)
    ...
    def test_data_validation_and_constraints(self) -> bool:
        ...
        try:
            ...
            large_payload_handled = status in [
                200, 201, 413, 422]  # Success or proper rejection
            ...
            validation_passed = (empty_type_rejected and null_type_rejected and
                                 large_payload_handled and future_timestamp_rejected)

            details = f"Empty type: {empty_type_rejected}, Null type: {null_type_rejected}, " \
    ...
    def test_api_rate_limiting(self) -> bool:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(make_rapid_request, i)
            ...
            details = f"Success: {success_count}, Rate limited: {rate_limited_count}, " \
                f"Errors: {error_count} out of {request_count} requests"

            self._log_test_result("API Rate Limiting",
                                  rate_limiting_works, details)
    ...
    def test_api_response_formats(self) -> bool:
        ...
        try:
            ...
            if status == 200 and response:
                event = response[0] if isinstance(
                    response, list) and response else response

                # Check required fields
                required_fields = ['id', 'user_id',
                                   'timestamp', 'event_type', 'value']
                rest_format_valid = all(
                    field in event for field in required_fields)

                # Check data types
                rest_types_valid = (
                    isinstance(event.get('id'), str) and
                    isinstance(event.get('user_id'), str) and
                    isinstance(event.get('timestamp'), str) and
                    isinstance(event.get('event_type'), str) and
                    isinstance(event.get('value'), dict)
                )
            else:
            ...
            if status == 200 and 'data' in response and response['data']['engagement_events']:
                event = response['data']['engagement_events'][0]

                # Check GraphQL response structure
                graphql_format_valid = all(
                    field in event for field in required_fields)
                graphql_types_valid = (
                    isinstance(event.get('id'), str) and
                    isinstance(event.get('user_id'), str) and
            ...
            format_passed = rest_format_valid and rest_types_valid and graphql_format_valid and graphql_types_valid

            details = f"REST format: {rest_format_valid}, REST types: {rest_types_valid}, " \
                f"GraphQL format: {graphql_format_valid}, GraphQL types: {graphql_types_valid}"

            self._log_test_result("API Response Formats",
                                  format_passed, details)
    ...
    def _generate_api_report(self):
        ...
        report_file = f"api_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

        report = {
            'test_suite': 'BEE Engagement Events API Validation',
            'timestamp': datetime.now().isoformat(),
            'config': {k: v for k, v in self.config.items() if 'key' not in k and 'jwt' not in k},
        ...
        print(
            f"Detailed API validation report saved to: tests/api/{report_file}")
