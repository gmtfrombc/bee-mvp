...
class TestInterventionEngine:
    ...
    async def test_consecutive_needs_care_trigger(
        ...
        interventions = supabase_client.table('coach_interventions')\
            .select('*')\
        ...
        notifications = supabase_client.table('momentum_notifications')\
            .select('*')\
            .eq('user_id', test_user_id)\
    ...
    async def test_rate_limiting_prevents_spam(
        ...
        result1 = await self._call_intervention_engine(test_user_id)
        result2 = await self._call_intervention_engine(test_user_id)
    ...
    @pytest.mark.asyncio
    async def test_bulk_user_processing(
        self,
        supabase_client: Client,
        clean_test_data
    ):
